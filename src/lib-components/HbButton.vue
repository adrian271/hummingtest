      ...
      start
            ...
            <v-tooltip bottom :disabled="disabled">
                <template v-slot:activator="{ on }">
                    <v-hover v-slot:default="{ hover }">
                        <v-btn :id="id" :to="to" :height="height" :href="href" :target="target" medium class="hb-v-btn-icon" :ripple="false" :disabled="disabled" icon v-on="on" :class="{'hb-button-icon-hover' : hover && !hoverBackgroundColor, 'hb-button-icon-active' : button && !activeBackgroundColor}" @click="buttonClicked" :style="(hoverBackgroundColor && hover) || (activeBackgroundColor && button) ? 'background-color:' + overwriteDefaultColors : ''">
                            <hb-icon
                                :hover="hover"
                                :toggle="toggle"
                                :small="small"
        ...
        <span v-else-if="type === 'icon' && !icon">
            <v-tooltip bottom :disabled="disabled">
                <template v-slot:activator="{ on }">
                    <v-hover v-slot:default="{ hover }">
                        <v-btn :id="id" :to="to" :href="href" :target="target" medium class="hb-v-btn-icon" :ripple="false" :disabled="disabled" icon v-on="on" :class="{'hb-button-icon-hover' : hover && !hoverBackgroundColor, 'hb-button-icon-active' : button && !activeBackgroundColor}" @click="buttonClicked" :style="(hoverBackgroundColor && hover) || (activeBackgroundColor && button) ? 'background-color:' + overwriteDefaultColors : ''">
        ...
        <span v-else>
            <v-hover v-slot:default="{ hover }">
                <v-btn :id="id" :to="to" :href="href" :target="target" :small="sizeSmall" :loading="loading" :large="sizeLarge" :block="block" :min-width="minWidth" :class="[ buttonClass, hover ? hoverClass : '' ]" depressed :ripple="false" :disabled="disabled" @click="$emit('click')">
                    <v-icon left v-if="prependIcon" :size="small ? '16' : large ? '24' : '20'" :color="color === 'secondary' ? '#637381' : '#FFFFFF'" :class="small ? 'mr-1' : 'mr-2'">{{ prependIcon }}</v-icon>
                    <span :class="[ textSize, textColor ]" class="font-weight-regular">
                        <slot></slot>
                    </span>
                    <v-icon right v-if="appendIcon" :size="small ? '16' : large ? '24' : '20'" :color="color === 'secondary' ? '#101318' : '#FFFFFF'" :class="small ? 'ml-0' : 'ml-1'">{{ appendIcon }}</v-icon>
                </v-btn>
            </v-hover>
        </span>
        finish
